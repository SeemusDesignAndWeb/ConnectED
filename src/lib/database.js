	...
	try {
		if (!existsSync(dbPath)) {
			...
			if (isProduction) {
				...
				throw new Error(`Database file not found at ${dbPath}. Please ensure the Railway volume is mounted and initialize the database.`);
			}
		...
		if (isProduction) {
			...
			throw new Error(`Failed to read database at ${dbPath}. Please check file permissions and volume mount.`);
		}
...
export function writeDatabase(data) {
	...
	try {
		...
		if (isProduction) {
			...
			throw new Error(`Failed to write database at ${dbPath}. Please check file permissions and volume mount.`);
		}
